...
const depositIds = computed(() =>
  store.loginUser.deposits.map((item) => item.deposit)
);
const deposits = computed(() =>
  destore.deposits.filter((item) => depositIds.value.includes(item.id))
);
const depositNames = computed(() =>
  deposits.value.map((item) => item.fin_prdt_nm)
);
const depositOptions = computed(() =>
  destore.depositoptions.filter((item) =>
    depositIds.value.includes(item.deposit)
  )
);
const depositRates = computed(() => {
  ...
  for (const key in groupedDeposit) {
    const avg =
      groupedDeposit[key].reduce((acc, cur) => acc + cur, 0) /
      groupedDeposit[key].length;
    result.push(avg);
  }
  return result;
});
