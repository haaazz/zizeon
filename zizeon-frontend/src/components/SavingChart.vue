...
const savingIds = computed(() =>
  store.loginUser.savings.map((item) => item.saving)
);
const savings = computed(() =>
  sastore.savings.filter((item) => savingIds.value.includes(item.id))
);
const savingNames = computed(() =>
  savings.value.map((item) => item.fin_prdt_nm)
);
const savingOptions = computed(() =>
  sastore.savingoptions.filter((item) => savingIds.value.includes(item.saving))
);
const savingRates = computed(() => {
  ...
  for (const key in groupedSaving) {
    const avg =
      groupedSaving[key].reduce((acc, cur) => acc + cur, 0) /
      groupedSaving[key].length;
    result.push(avg);
  }
  return result;
});
