...
const filterSavings = () => {
  if (selectedBank.value) {
    filteredSavings.value = savings.value.filter(saving => saving.kor_co_nm === selectedBank.value)
  } else {
...
const getInterestRate = (saving, type, trm) => {
  const option = options.value.find(option => option.saving === saving.id && option.rsrv_type_nm === type && option.save_trm === trm)
